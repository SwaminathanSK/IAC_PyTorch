...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        """
        discounted_returns = np.sum(np.array(list((self.gamma)**t*rewards[t] for t in range(len(rewards)))))
        list_of_discounted_returns = np.ones(len(rewards))*discounted_returns
        # list_of_discounted_returns = np.array((self.gamma)**t*rewards[t] for t in range(len(rewards)))
        """
        END CODE
        """
    ...
    def _discounted_cumsum(self, rewards):
        ...
        """
        list_of_discounted_cumsums = np.array(list(np.sum(np.array(list((self.gamma)**(t_prime-t)*rewards[t_prime] for t_prime in range(t, len(rewards))))) for t in range(len(rewards))))
        """
        END CODE
        """
